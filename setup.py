...
#
# I plan to support two installation processes: end-users, which should be able to download and run a single Windows binary; then developers, which can ``easy_install CodeChat`` or ``pip install CodeChat``. Note that these require manual installation of Python 2.7 and PyQt to work. This script is only useful for creating a nice package for developers; PyInstaller produces the end-user binary.
#
...
setup(name = "CodeChat",
      ...
      data_files = [('Lib/site-packages', ['tre.dll'])],
